...
pub async fn login(account: &str, brand: Brand) -> Result<()> {
    state::validate_name(account)?;
    if state::account(account)?.is_some() {
        bail!(
    ...
    state::save_account(account, &created)?;
    println!(
...
pub async fn login_existing(
    ...
    state::save_account(account, &credentials)?;
...
pub async fn poll(
    ...
    loop {
        ...
        let owner = answer
            ...
            .and_then(Value::as_str)
            .filter(|open_id| state::validate_open_id(open_id).is_ok())
            .map(str::to_owned);
        if owner.is_none() {
