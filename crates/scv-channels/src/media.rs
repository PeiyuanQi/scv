...
pub(crate) const MAX_REPLY_FILES: usize = 8;
/// Files one received message may bring into a turn.
pub(crate) const MAX_MESSAGE_MEDIA: usize = 16;
...
impl MediaKind {
    ...
    pub(crate) fn for_mime(mime: &str) -> Self {
...
impl MediaSettings {
    ...
    pub(crate) fn limit(&self, kind: MediaKind, owner: bool) -> Option<u64> {
        let mib = if owner {
            self.owner_max_mib
    ...
    pub(crate) fn keep(&self) -> Duration {
...
pub(crate) struct MediaOptions {
    ...
    pub(crate) settings: MediaSettings,
}
...
pub(crate) fn mime_type(declared: Option<&str>, name: &str, head: &[u8]) -> String {
...
pub(crate) fn save(dir: &Path, name: &str, bytes: &[u8]) -> Result<PathBuf> {
    private_dir(dir)?;
...
pub(crate) fn prune(root: &Path, keep: Duration) -> usize {
...
/// cannot be read.
pub(crate) fn head(path: &Path) -> Vec<u8> {
    use std::io::Read;
