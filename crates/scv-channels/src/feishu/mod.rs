...
use crate::retry::{SendLabels, retry_send};
use crate::{
...
/// The channel name: `scv channels <command> feishu` (and `lark`).
pub const CHANNEL: &str = "feishu";

/// The Feishu channel, and Lark, its international edition, through a bot
/// app.
pub struct Feishu;

/// How a Feishu or Lark account signs in.
pub enum Login {
    /// Scan a QR code that creates a bot app; its creator is the owner.
    Scan { brand: Brand },
    /// Add an existing app by its ID and secret, which are checked with the
    /// platform before anything is saved.
    Existing {
        brand: Brand,
        app_id: String,
        app_secret: Secret,
        /// The owner's `open_id` for this app; without one, remote tools
        /// stay off for everyone.
...
impl Channel for Feishu {
    const KIND: ChannelKind = ChannelKind::Feishu;
    type Credentials = Account;
    type Login = Login;

    async fn login(layout: &Layout, account: &str, login: Login) -> Result<()> {
        let store = Store::new(layout, CHANNEL);
        match login {
            Login::Scan { brand } => login::login(&store, account, brand).await,
            Login::Existing {
                brand,
                app_id,
                app_secret,
    ...
    fn owner(credentials: &Account) -> Option<&str> {
        credentials.owner_open_id.as_deref()
    }

    /// The bot app.
    fn bot_id(credentials: &Account) -> Option<String> {
        Some(credentials.app_id.clone())
    }

    fn title(credentials: &Account) -> &'static str {
        credentials.brand.title()
    }

    /// Only a live connection reports healthy.
    async fn run(run: AccountRun<'_>, credentials: &Account) -> Result<()> {
        ...
        let transport = OpenPlatform::new(Endpoints::for_brand(credentials.brand), credentials)?;
...
impl OpenPlatform {
    pub(crate) fn new(endpoints: Endpoints, credentials: &credentials::Account) -> Result<Self> {
        Ok(Self {
...
#[async_trait]
impl Transport for OpenPlatform {
