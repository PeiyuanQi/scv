...
use crate::{
    AccountRun, Batch, Channel, ChannelKind, Downloaded, Inbound, Media, Message, Outbound,
    OutboundFile, SendOutcome, Transport,
};
...
/// The WeChat channel, through a ClawBot (iLink) bot.
pub struct WeChat;

/// How a WeChat account signs in: by scanning a ClawBot QR code served from
/// `base_url` (normally `https://ilinkai.weixin.qq.com`).
pub struct Login {
    pub base_url: String,
}
...
impl Channel for WeChat {
    const KIND: ChannelKind = ChannelKind::Wechat;
    type Credentials = Account;
    type Login = Login;

    async fn login(layout: &Layout, account: &str, login: Login) -> Result<()> {
        self::login(&Store::new(layout, CHANNEL), &login.base_url, account).await
    }
    ...
    /// Only a validated authenticated getupdates response reports healthy.
    async fn run(run: AccountRun<'_>, credentials: &Account) -> Result<()> {
        ...
        serve(credentials.token.expose(), &base_url, bridge, &store).await
    }
}

/// Sign `account` in by QR code and save it in `store`.
async fn login(store: &Store, base: &str, account: &str) -> Result<()> {
    ...
    loop {
        ...
        {
            "confirmed" => {
                let (token, bot_id, user_id) = ilink::validate_confirmed_login(&status)?;
                ...
                ilink::validate_origin_pair(&base, &host)?;
                store.save_account(
                    account,
                    &Account {
                        token: token.into(),
...
async fn serve(
    ...
    crate::serve(&transport, run, store, |saved| {
        credentials::runs_as(saved, token, base_url)
    })
    .await
}
...
impl Transport for Ilink<'_> {
    ...
    async fn send(
        ...
        let body = ilink::reply_body(message.to, context_token, message.text, message.client_id);
        ilink::send_reply_request(&self.client, self.token, self.base_url, &body, report).await
    }

    async fn download(&self, media: &Media, max_bytes: u64) -> Result<Downloaded> {
        let source: cdn::Source =
            serde_json::from_str(&media.source).map_err(|_| anyhow!("bad media source"))?;
        let bytes = cdn::download(&self.client, &source, max_bytes).await?;
    ...
    async fn send_file(
        ...
        .await??;
        let upload = cdn::Upload::new(&plain, file.kind);
        drop(plain);
        let Some(download_param) = ilink::upload(
            &self.client,
            self.token,
            self.base_url,
            file.to,
    ...
    for item in items {
        if let Some(quoted) = item.get("ref_msg") {
            ...
            if let Some(quoted_item) = quoted.get("message_item") {
                if let Some(label) = cdn::item_label(quoted_item) {
                    parts.push(label);
                }
                files.extend(cdn::item_media(quoted_item));
            }
            if !parts.is_empty() {
        ...
        match item.get("type").and_then(Value::as_i64) {
            ...
            _ => files.extend(cdn::item_media(item)),
        }
    }
    let sender = msg
        .get("from_user_id")
...
pub(crate) fn check_envelope(value: &serde_json::Value) -> Result<()> {
    let ret = value
        .get("ret")
...
pub(crate) fn check_send_ack(body: &[u8]) -> std::result::Result<(), String> {
