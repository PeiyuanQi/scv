...
pub(crate) async fn upload(
    ...
    base_url: &str,
    to_user_id: &str,
    upload: &crate::wechat::cdn::Upload,
    ...
    retry_send(labels, report, || async {
        let result = async {
            let response = client
                ...
                .timeout(crate::wechat::REQUEST_TIMEOUT)
                .send()
                .await?;
            ...
            let body = crate::wechat::response_body(response).await?;
            if let Err(rejection) = crate::wechat::check_send_ack(&body) {
                return Ok(Attempt::Refused(format!(
            ...
            let response = client
                ...
                .timeout(crate::wechat::CDN_TIMEOUT)
                .send()
                .await
...
pub(crate) async fn send_reply_request(
    ...
    let sent = retry_send(labels, report, || async {
        let result = async {
            ...
            let body = crate::wechat::response_body(response).await?;
            Ok::<_, anyhow::Error>(match crate::wechat::check_send_ack(&body) {
                Ok(()) => Attempt::Done(()),
...
pub(crate) fn parse_response(body: &[u8]) -> Result<Value> {
    ...
    crate::wechat::check_envelope(&value)?;
    Ok(value)
}
