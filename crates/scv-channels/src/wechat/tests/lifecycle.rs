//! Lifecycle tests for `src/wechat/mod.rs`: the WeChat bridge against a fake
//! iLink server and a fake SCV daemon.

use super::*;
use crate::{
    BUSY_REPLY, FAILURE_REPLY, HELD_HEADER, LATEST_HEADER, MAX_CONCURRENT_TURNS,
    MAX_QUEUED_PER_CONVERSATION, MAX_REPLY_BYTES, MAX_TOTAL_REPLY_BYTES, ToolOwner, new_pending,
    owner_turn_timeout, recover_interrupted,
};
use serde_json::json;
use std::os::fd::AsFd;
use std::path::Path;
...
/// Run `work` until `cancellation` fires, as the daemon runs an account.
...
impl<'a> Bridge<'a> {
    ...
    fn link(mut self, link: &'a crate::hub::Link) -> Self {
        self.link = Some(link);
        self
    }
    ...
    store
        .save_state(
            ...
            &crate::state::BridgeState {
                ...
                in_flight: vec![crate::state::InFlight {
...
async fn recovery_answers_every_claim_once_and_never_replays_it() {
    ...
    let mut ilink = FakeIlink::start().await;
    let store = saved_store(directory.path(), &ilink.base);
    let claim = |id: &str, key: &str| crate::state::InFlight {
        message_id: id.into(),
        to_user_id: "sender".into(),
...
async fn after_a_planned_restart_interrupted_work_is_described_as_such() {
    use crate::hub::{Hub, Link, Restart};
    let directory = tempfile::tempdir().unwrap();
    let mut ilink = FakeIlink::start().await;
    let store = saved_store(directory.path(), &ilink.base);
    store
...
fn media_message(ilink: &FakeIlink, id: &str, sender: &str, key: &[u8; 16]) -> (Value, Vec<u8>) {
    ...
    let png = b"\x89PNG\r\n\x1a\nfake image".to_vec();
    let image_url = ilink.serve_file(
        &format!("{id}-image"),
        crate::wechat::cdn::encrypt(&png, key),
    );
    let file_url = ilink.serve_file(
        &format!("{id}-file"),
        crate::wechat::cdn::encrypt(b"%PDF-1.7", key),
    );
...
async fn files_the_model_attaches_are_uploaded_encrypted_after_the_reply() {
    ...
    let chart =
        crate::media::save(&options.outbox, "chart.png", b"\x89PNG\r\n\x1a\nchart").unwrap();
    ...
    let peer = async {
        ...
        let key =
            crate::wechat::cdn::tests_key(item["image_item"]["media"]["aes_key"].as_str().unwrap());
        let uploaded = ilink.uploads.lock().unwrap()[0].clone();
        assert_eq!(
            crate::wechat::cdn::decrypt(&uploaded, &key).unwrap(),
            b"\x89PNG\r\n\x1a\nchart"
        );
...
async fn a_restart_resumes_sending_files_where_it_stopped_with_the_same_client_id() {
    ...
    let store = saved_store(directory.path(), &ilink.base);
    let options = media(directory.path());
    let first = crate::media::save(&options.outbox, "a.txt", b"first").unwrap();
    let second = crate::media::save(&options.outbox, "b.txt", b"second").unwrap();
    let file = |path: &Path, name: &str, client_id: &str| crate::state::PendingFile {
        path: std::fs::canonicalize(path).unwrap().display().to_string(),
        name: name.into(),
        mime: "text/plain".into(),
        kind: crate::MediaKind::File,
