...
`scv-provider-openai`, `scv-tools`, `scv-clawbot`, `scv-server`, `scv-tui`, and
`scv-cli` in that order, and skips any crate already on crates.io, so a partial
run can be resumed. It needs the `cargo login` token in the real home.
