...
live daemon, and returns. `stop` persistently disables the account and joins
...
account's identity, enabled setting, health state, restart count, sanitized
error, and last successful authenticated, validated `getupdates` timestamp.
...
Remote sessions use `session.start` with `no_tools: true`, enforced by the
server, and deny any approval requests. The bridge never invokes `scv exec --yes`.
