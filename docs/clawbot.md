...
`login` renders the QR code and stores credentials without printing the token.
`run` validates the account and workspace, reconnects with bounded backoff, and
denies remote approval requests. `status` reports whether credentials are
saved. `logout` removes local credentials and state. The API has no documented
...
accepted messages are marked only after their pending reply is durable.
...
Network failures use bounded exponential backoff. Duplicate inbound
...
sessions, and delivery retries. Unit tests cover response validation, origin
selection, state compatibility, and UTF-8 chunking. No test contacts WeChat.
